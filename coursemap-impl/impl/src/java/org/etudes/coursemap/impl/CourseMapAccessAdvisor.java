/**********************************************************************************
 * $URL: https://source.etudes.org/svn/apps/coursemap/trunk/coursemap-impl/impl/src/java/org/etudes/coursemap/impl/CourseMapAccessAdvisor.java $
 * $Id: CourseMapAccessAdvisor.java 1752 2011-07-06 03:24:32Z ggolden $
 ***********************************************************************************
 *
 ...
 */
public class CourseMapAccessAdvisor implements AccessAdvisor
{
	/** Our logger. */
	private static Log M_log = LogFactory.getLog(CourseMapAccessAdvisor.class);

	/** Thread local key to blocking the advisor's activity. */
	protected final static String BLOCK = "CourseMapAccessAdvisor.block";
	 ...
	 * {@inheritDoc}
	 */
	public Boolean denyAccess(String toolId, String context, String id, String userId)
	{
		// check for being blocked
		if (this.threadLocalManager.get(BLOCK) != null) return Boolean.FALSE;

		// guests don't get blocked
		if (this.courseMapService.isGuest(context, userId)) return Boolean.FALSE;
		...
		for (CourseMapItem item : map.getItems())
		{
			...
			{
				// this is the item... if it is blocked, we deny access
	 ...
	 * {@inheritDoc}
	 */
	public String details(String toolId, String context, String id, String userId)
	{
		// check for being blocked
		if (this.threadLocalManager.get(BLOCK) != null) return null;

		// guests don't get blocked
		if (this.courseMapService.isGuest(context, userId)) return null;
		...
		for (CourseMapItem item : map.getItems())
		{
			...
			if ((item.getType().getAppCode() == type.getAppCode()) && (item.getId().equals(id)))
			{
				// this is the item... if it is blocked, get the details about the blocker
				String blockerTitle = item.getBlockedByDetails();
				return blockerTitle;
			}
		}
	 ...
	 * {@inheritDoc}
	 */
	public String message(String toolId, String context, String id, String userId)
	{
		// check for being blocked
		if (this.threadLocalManager.get(BLOCK) != null) return null;
		...
		for (CourseMapItem item : map.getItems())
		{
			...
			if ((item.getType().getAppCode() == type.getAppCode()) && (item.getId().equals(id)))
			{
				// this is the item... if it is blocked, get the title of the blocker
				String blockerTitle = item.getBlockedByTitle();
				return blockerTitle;
			}
		}
